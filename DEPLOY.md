...
### POST だけ失敗する（GET は動くのにサインアップができない）

Vercel の Node ランタイムは、ハンドラを呼ぶ前にリクエストボディを読み切って
`req.body` に入れることがある。素の `req` をそのまま渡すとストリームは空で、
Hono の `parseBody()` が何も取れず「入力が空」として弾かれる。

サインアップなら「メールアドレスの形式が正しくありません」が出る。
正しいアドレスを入れているのにこれが出たら、この症状。

`api/index.ts` で、先読みされていた場合に同じ内容のストリームを作り直している。
その際 `headers` だけでなく **`rawHeaders` も引き継ぐこと**。
@hono/node-server は `rawHeaders` を読むため、片方だけだと別のエラーになる。
